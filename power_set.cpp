...
// function to print a sequence, given the number of elements the actual sequence stored as an array void printPowerset(int, int*, int&, float*);
// function to generate the power set of {1, 2, ...first argument} and retrieve the best set void checkSet(int[], int, int*, int&, float*);
...
cout << "Longest non­decreasing subsequence problem, powerset algorithm" << endl; cout << "Enter the number of elements in the sequence" << endl;
...
// retrieve the indices for generating the subsequence for(i=0;i<bestSize;i++)
// decrease each index by one since the indices of array A are in // the range 0..n­1 and not 1..n
R[i]=A[bestSet[i+1]­1];
...
cout << bestSize << endl;
cout << "The longest non­decreasing subsequence is" << endl; print_sequence(bestSize, R);
...
// function to print a sequence, given the number of elements the actual sequence stored as an array // n represents the number of elements in the sequence
...
// function to generate the power set of {1, .., n} and retrieve the best set // n represents the maximum value in the set
// bestSet represents the set
// bestSize is the size of the bestSet
int *stack,k;
// allocate space for the set
stack = new int[n+1];
stack[0]=0; /* 0 is not considered as part of the set */ k = 0;
while(1){
if (stack[k]<n){
...
// deallocate space for the set delete [ ] stack;
return;
}
void checkSet(int *stack, int k, int *bestSet, int &bestSize, float *A)
// function to check the currently generated set stack of size k against the current // best set bestSet of size bestSize
{
int i;
// check that the indices in stack generate a subsequence of non­decreasing order if (k <= 2) {
// the set contains a single index so the subsequence is in non­decreasing order if (k > bestSize) {
// we found a better set
// WRITE CODE TO STORE stack into bestSet and UPDATE bestSize TO k return;
} }
else {
// the set contains more than a single index so check that the subsequence is in order for(i=0;i<k­1;i++) {
// decrease each index by one since the indices of array A are in
// the range 0..n­1 and not 1..n
// WRITE CODE (AN IF STATEMENT) TO CHECK THAT THE ELEMENTS IN ARRAY // A AT INDICES stack[i+1]­1 AND stack[i+2]­1 ARE IN NON­DECREASING ORDER
// IF THE TWO ELEMENTS ARE OUT OF ORDER THEN return
} }
// we have an non­decreasing so we compare it against the current best set if (k > bestSize) {
// we found a better set
// WRITE CODE TO STORE stack into bestSet and UPDATE bestSize TO k return;
} else
return; }
